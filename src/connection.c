...
void configure_connection(MYSQL *conn) {
  ...
  g_message("MYSQL_READ_DEFAULT_FILE = %s", connection_defaults_file);
  g_message("MYSQL_READ_DEFAULT_GROUP = %s", connection_default_file_group);
...
void smd_connect(MYSQL *conn, const gchar *app, gchar *schema){
  connection_default_file_group=app;
  m_connect(conn, schema);
}
